...
void LauncherList::Process() {
	...
	while(cur != m_pendingLaunchers.end()) {
		...
		if(!l->Process()) {
			...
			if (l != nullptr)
			{
				Log(Logs::Detail, Logs::WorldServer, "Removing pending launcher %d", l->GetID());
			}
			else
			{
				Log(Logs::Detail, Logs::WorldServer, "Removing pending launcher with no known ID");
			}
	...
	while(curl != m_launchers.end()) {
		...
		if(!l->Process()) {
			//launcher has died before it identified itself.
			Log(Logs::Detail, Logs::WorldServer, "Removing launcher %s (%d)", l->GetName(), l->GetID());
