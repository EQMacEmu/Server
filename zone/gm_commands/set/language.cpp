...
void SetLanguage(Client *c, const Seperator *sep)
{
	...
	if (arguments < 3 || !sep->IsNumber(2) || !sep->IsNumber(3)) {
		...
		c->Message(Chat::White, "Language ID = 0 to 26");
	...
	if (
		!EQ::ValueWithin(language_id, Language::CommonTongue, Language::Unknown26) ||
		!EQ::ValueWithin(language_value, 0, Language::MaxValue)
	) {
		c->Message(Chat::White, "Usage: #set language [Language ID] [Language Value]");
