...
void command_npcspawn(Client *c, const Seperator *sep){
	...
	if (is_add || is_create) {
		...
		c->Message(Chat::White, fmt::format("Spawn {} | Name: {} ({})", is_add ? "Added" : "Created", target->GetCleanName(), target->GetID()).c_str());
	}
		...
		c->Message(Chat::White, fmt::format("Spawn {} | Name: {} ({})", spawn_message, target->GetCleanName(), target->GetID()).c_str());
