...
float Map::FindBestZ(glm::vec3 &start, glm::vec3 *result, float limit, float offset) const {
	...
	if (offset > 0.0f && offset < 5.0f) {
		size_adjust = 5.0f - offset;
	}
	float start_z = start.z + size_adjust;
...
float Map::FindClosestZ(glm::vec3 &start, glm::vec3 *result, float z_offset) const {
	...
	if (hit) {
		if (ClosestZ != BEST_Z_INVALID) {
			float origin = start.z - z_offset;
			if (std::abs(result->z - origin) > std::abs(origin - ClosestZ))
				return ClosestZ;
		}
...
Map *Map::LoadMapFile(std::string file) {
	...
	std::string filename = fmt::format("{}/{}.map", PathManager::Instance()->GetMapsPath(), file);
...
bool Map::NoHazardsAccurate(glm::vec3 From, glm::vec3 To, float size, int max_steps, float interval)
{
	...
	glm::vec3 step(end.x - begin.x, end.y - begin.y, end.z - begin.z);

	float last_z = begin.z;
	float factor = glm::length(end - begin);

	float step_size = interval;
	if (interval < 1.0f)
		step_size = 1.0f;

	float total_steps = factor / step_size;

	int count = (int)total_steps;
	if (max_steps > 0 && total_steps > (float)max_steps) {
		count = max_steps;
	}
	...
	for (int i = 0; i < count; i++)
	{
		...
		if (std::abs(NewZ - last_z) > haz_size) {
			// possible hazard
			if (zone->watermap && NewZ > -2000.0f && (NewZ < last_z)) {
				if (zone->watermap->InLiquid(begin)) {
					continue;
				}
			}
			return false;
	...
	if (count < max_steps && std::abs(last_z - end.z) > haz_size)
		return false;
	...
	for (uint32 i = 0; i < plac_group_count; ++i) {
		...
		for (uint32 j = 0; j < p_count; ++j) {
			...
			for (size_t k = 0; k < model->polys.size(); ++k) {
				...
				RotateVertex(v1, x_rot * 3.141592654f / 180.0f, 0, 0);
				RotateVertex(v2, x_rot * 3.141592654f / 180.0f, 0, 0);
				RotateVertex(v3, x_rot * 3.141592654f / 180.0f, 0, 0);

				RotateVertex(v1, 0, y_rot * 3.141592654f / 180.0f, 0);
				RotateVertex(v2, 0, y_rot * 3.141592654f / 180.0f, 0);
				RotateVertex(v3, 0, y_rot * 3.141592654f / 180.0f, 0);

				glm::vec3 correction(p_x, p_y, p_z);

				RotateVertex(correction, x_rot * 3.141592654f / 180.0f, 0, 0);
				...
				RotateVertex(v1, 0, 0, z_rot * 3.141592654f / 180.0f);
				RotateVertex(v2, 0, 0, z_rot * 3.141592654f / 180.0f);
				RotateVertex(v3, 0, 0, z_rot * 3.141592654f / 180.0f);
