...
float GetReciprocalHeading(const float heading)
{
	...
	// Convert to radians
	float h = (heading / 256.0f) * 6.283185308f;

	// Calculate the reciprocal heading in radians
	result = h + 3.141592654f;

	// Convert back to eq heading from radians
	result = (result / 6.283185308f) * 256.0f;
...
float CalculateHeadingAngleBetweenPositions(float x1, float y1, float x2, float y2)
{
	...
	if (y1 >= y2) {
		if (x2 >= x1)
			return ((90.0f - angle + 90.0f) * 256.0f * 0.0027777778f);
		if (x2 <= x1)
			return ((angle + 180.0f) * 256.0f * 0.0027777778f);
	}
	if (y1 > y2 || x2 > x1)
		return (angle * 256.0f * 0.0027777778f);
	else
		return ((90.0f - angle + 270.0f) * 256.0f * 0.0027777778f);
}
