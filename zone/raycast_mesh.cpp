...
static inline bool rayIntersectsTriangle(const RmReal *p,const RmReal *d,const RmReal *v0,const RmReal *v1,const RmReal *v2,RmReal &t)
{
	...
	glm::vec3 e2(v2[0] - v0[0], v2[1] - v0[1], v2[2] - v0[2]);

	//vector(e1,v1,v0);
	//vector(e2,v2,v0);
	
	glm::vec3 h = glm::cross(glm::vec3(d[0], d[1], d[2]), e2);
	// crossProduct(h,d,e2);
	a = glm::dot(e1, h);
	...
	f = 1/a;
	glm::vec3 s(p[0] - v0[0], p[1] - v0[1], p[2] - v0[2]);
	//vector(s,p,v0);
	u = f * glm::dot(s, h);
	...
	//crossProduct(q,s,e1);
	v = f * glm::dot(glm::vec3(d[0],d[1],d[2]), q);
...
static RmReal computePlane(const RmReal *A,const RmReal *B,const RmReal *C,RmReal *n) // returns D
{
	...
	RmReal mag = glm::sqrt((vw_x * vw_x) + (vw_y * vw_y) + (vw_z * vw_z));
...
public:
	...
	virtual bool raycast(const RmReal *from,const RmReal *to,RmReal *hitLocation,RmReal *hitNormal,RmReal *hitDistance)
	{
		...
		RmReal dir[3] = { to[0] - from[0], to[1] - from[1], to[2] - from[2] };
		RmReal distance = glm::sqrt( dir[0]*dir[0] + dir[1]*dir[1] + dir[2]*dir[2] );
		if ( distance < 0.0000000001f ) return false;
		RmReal recip = 1.0f / distance;
		...
		mRoot->raycast(ret,from,to,(const RmReal*)&dir,hitLocation,hitNormal,hitDistance,mVertices,mIndices,distance,this,mRaycastTriangles,mRaycastFrame,mLeafTriangles,nearestTriIndex);
		return ret;
	}
