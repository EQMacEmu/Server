...
RULE_BOOL (Character, PlayerTradingLoreFeedback, false, "If enabled, during a player to player trade, if lore items exist, it will output which items.")
RULE_CATEGORY_END()
...
RULE_INT  (AlKabor, PlayerPopulationCap, 1200, "Max Players allowed in the Server. Will exclude offline Bazaar trader characters.")
RULE_INT  (AlKabor, TestPopulationOffset, 0, "Test population offset for queue testing")
RULE_INT  (AlKabor, IPDatabaseSyncInterval, 300, "Interval in seconds between database synchronization of IP reservation data")
RULE_INT  (AlKabor, MaxPlayersOnline, 1200, "Maximum number of players allowed online before queue is activated")
RULE_BOOL (AlKabor, EnableQueue, true, "Enable the login queue system when server population is at capacity")
RULE_BOOL (AlKabor, QueueBypassGMLevel, true, "Allow GMs (status >= 80) to bypass the queue")
RULE_BOOL (AlKabor, EnableQueueLogging, true, "Enable detailed logging for queue system operations")
RULE_BOOL (AlKabor, FreezeQueue, false, "Freeze queue advancement - players remain at current positions")
RULE_BOOL (AlKabor, EnableQueuePersistence, true, "Enable saving queue state to database for crash recovery")
RULE_INT  (AlKabor, DefaultGracePeriod, 60, "Default grace period in seconds for IP reservations when not specified")
RULE_INT  (AlKabor, IPCleanupInterval, 5, "Interval in seconds between IP reservation cleanup cycles")
RULE_CATEGORY_END()
...
RULE_INT(Aggro, ClientAggroCheckIdleInterval, 3000, "Interval in which clients actually check for aggro while idle - in milliseconds - this should be higher than ClientAggroCheckMovingInterval")
RULE_CATEGORY_END()
...
RULE_BOOL(Cheat, EnableMQWarpDetector, false, "Enable the MQWarp Detector. Set to False to disable this feature.")
...
RULE_BOOL(Cheat, EnableMQGhostDetector, false, "Enable the MQGhost Detector. Set to False to disable this feature.")
