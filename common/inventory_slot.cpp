...
{
	if ((!inventory_slot.Typeless() && !inventory_slot.IsTypeIndex(invtype::typePossessions)) || !inventory_slot.IsContainerIndex(invbag::SLOT_INVALID))
		return textures::textureInvalid;
...
{
	if (_typeless)
		return (_slot_index == invslot::SLOT_INVALID && _container_index == invbag::SLOT_INVALID);
	else
		return (_type_index == invtype::TYPE_INVALID && _slot_index == invslot::SLOT_INVALID && _container_index == invbag::SLOT_INVALID);
}
...
bool EQ::InventorySlot::IsEquipmentIndex(int16 slot_index)
{
	/*if (slot_index < inventory::EquipmentBegin || slot_index > inventory::EquipmentEnd)
		return false;*/
	if ((slot_index < invslot::EQUIPMENT_BEGIN || slot_index > invslot::EQUIPMENT_END) && slot_index != legacy::SLOT_QUEST)
		return false;
...
bool EQ::InventorySlot::IsGeneralSlot() const
{
	...
	return IsGeneralIndex(_slot_index);
}
...
{
	return InventorySlot(_type_index);
}

EQ::InventorySlot EQ::InventorySlot::ToOwner() const
{
	if (IsSlot() || IsContainer())
		return InventorySlot(_type_index);
...
{
	return StringFormat("(%i%s, %i, %i, %i)", _type_index, (_typeless ? " [typeless]" : ""), _slot_index, _container_index);
}

const std::string EQ::InventorySlot::ToName() const
{
	return StringFormat("InventorySlot - _type_index: %i%s, _slot_index: %i, _container_index: %i, _socket_index: %i", _type_index, (_typeless ? " [typeless]" : ""), _slot_index, _container_index);
}
...
//{
//	if ((slot_index >= inventory::EquipmentBegin) && (slot_index <= inventory::EquipmentEnd) && (slot_index != invslot::slotAmmo))
//		return true;
//
//	return false;
//}
...
//{
//	if ((_type_index != invtype::typePossessions) || (_container_index != invbag::SLOT_INVALID) || (_socket_index != inventory::socketInvalid))
//		return false;
//
...
bool EQ::operator==(const InventorySlot& lhs, const InventorySlot& rhs)
{
	if (lhs.Typeless() || rhs.Typeless())
		return ((lhs.SlotIndex() == rhs.SlotIndex()) && (lhs.ContainerIndex() == rhs.ContainerIndex()));
	
	return ((lhs.TypeIndex() == rhs.TypeIndex()) && (lhs.SlotIndex() == rhs.SlotIndex()) && (lhs.ContainerIndex() == rhs.ContainerIndex()));
}