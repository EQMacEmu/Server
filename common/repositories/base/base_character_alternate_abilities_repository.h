...
public:
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			e.aa_id    = static_cast<uint16_t>(strtoul(row[2], nullptr, 10));
			e.aa_value = static_cast<uint16_t>(strtoul(row[3], nullptr, 10));
	...
	)
	{
		...
		v.push_back(columns[2] + " = " + std::to_string(e.aa_id));
		v.push_back(columns[3] + " = " + std::to_string(e.aa_value));
