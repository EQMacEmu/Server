...
namespace EQ
{
	...
	public:
		...
		InventoryProfile() { m_mob_version = EQ::versions::MobVersion::Unknown; m_mob_version_set = false; }
		~InventoryProfile();

		bool SetInventoryVersion(versions::MobVersion inventory_version);
		bool SetInventoryVersion(EQ::versions::ClientVersion client_version) { return SetInventoryVersion(EQ::versions::ConvertClientVersionToMobVersion(client_version)); }
	...
	protected:
		...
		int16 _HasItem(std::map<int16, ItemInstance*>& bucket, int16 item_id, uint8 quantity);
